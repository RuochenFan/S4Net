...
class coco(imdb):
  ...
  def _print_detection_eval_metrics(self, coco_eval, logger):
    IoU_lo_thresh = 0.5
    IoU_hi_thresh = 0.95
  ...
  def _do_detection_eval(self, res_file, output_dir, logger):
    ann_type = 'bbox'
  ...
  def _write_coco_results_file(self, all_boxes, res_file, logger):
    # [{"image_id": 42,
    #   "category_id": 18,
  ...
  def evaluate_detections(self, all_boxes, output_dir, logger):
    ...
    res_file += '.json'
    self._write_coco_results_file(all_boxes, res_file, logger)
