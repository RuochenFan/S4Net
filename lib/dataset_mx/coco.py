...
from dataset_mx.utils.mask_coco2voc import mask_coco2voc
from dataset_mx.utils.mask_voc2coco import mask_voc2coco
# from bbox.bbox_transform import clip_boxes
from detection_opr.utils.bbox_transform import clip_boxes
...
def generate_cache_seg_inst_kernel(annWithObjs):
    ...
    ann = annWithObjs[0]
    objs = annWithObjs[1]
    gt_mask_file = ann['cache_seg_inst']    # just like /home/fanruochen/tf-detection/data/cache/COCOMask/train2014/COCO_train2014_000000262145.hkl
    if not gt_mask_file:
        return
    ...
    if not os.path.exists(gt_mask_file):
        with open(gt_mask_file, 'wb') as fid:
            hkl.dump(gt_mask.astype('bool'), fid)
    # cache flip gt_masks
    if not os.path.exists(gt_mask_flip_file):
        with open(gt_mask_flip_file, 'wb') as fid:
            hkl.dump(gt_mask[:, :, ::-1].astype('bool'), fid)

class coco(IMDB):
    def __init__(self, image_set, root_path, data_path, result_path=None,
                 mask_size=-1, binary_thresh=0.5):
        """
    ...
    def gt_roidb(self):
        ...
        gt_roidb = [self._load_coco_annotation(index) for index in
    ...
    def gt_sdsdb(self):
        ...
        # pool.close()
        # pool.join()
        for i in range(len(gt_sdsdb_temp)):
            print(i)
            generate_cache_seg_inst_kernel(gt_sdsdb_temp[i])
    ...
    def mask_path_from_index(self, index):
        ...
        if self.data_name == 'val':
            return []
        cache_file = os.path.join(self.cache_path, 'COCOMask', self.data_name, '%d' % (index % 100))
    ...
    def _write_coco_results(self, all_boxes, res_file, ann_type, all_masks):
        ...
        if ann_type == 'bbox':
            data_pack = [{'cat_id': self._class_to_coco_ind[cls],
                         ...
                         for cls_ind, cls in enumerate(self.classes) if
                         not cls == '__background__']
        elif ann_type == 'segm':
            data_pack = [{'cat_id': self._class_to_coco_ind[cls],
                          'cls_ind': cls_ind,
                          'cls': cls,
